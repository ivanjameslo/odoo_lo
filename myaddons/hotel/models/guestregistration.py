    ...
    def action_reserve(self):
        for rec in self:
            if not(rec.guest_id):  
                ...
                raise ValidationError('Please supply the Scheduled Check In date.')
            elif not(rec.datetoSched):    
                raise ValidationError('Please supply the Scheduled Check Out date.')
            elif rec.datefromSched > rec.datetoSched:
                raise ValidationError('Scheduled Check In date cannot be greater than Scheduled Check Out date.')         
            else:
                pkid = rec.id
